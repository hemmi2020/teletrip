...
const HotelSearchForm = () => {
  ...
  // Fetch all countries and cities on mount
  useEffect(() => {
        ...
        if (!data.error) {
          ...
          data.data.forEach(country => {
            ...
            locations.push({
              ...
              searchText: country.country.toLowerCase()
            });
  ...
  const handleSubmit = (e) => {
    ...
    // Add city only if it's a city selection
    if (selectedLocation.type === 'city' && selectedLocation.city) {
      url += `&city=${encodeURIComponent(selectedLocation.city)}`;
    } else {
      // If country only is selected, use the country name as the city for geocoding
      url += `&city=${encodeURIComponent(selectedLocation.country)}`;
    }
  ...
  return (
    <div className=".sm:min-h-screen  md:pt-30 sm:pt-8 bg-transparent p-2 sm:p-4 ">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white/30 rounded-xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8 backdrop-blur-lg bg-opacity-95 ">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 mb-4 sm:mb-6 md:mb-8">
            Search for hotels, transfers, experiences and more
          </h1>

          {/* Tabs */}
          <div className="flex overflow-x-auto space-x-1 sm:space-x-2 mb-4 sm:mb-6 md:mb-8 pb-2 scrollbar-hide">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex items-center space-x-1 sm:space-x-2 px-2 sm:px-3 md:px-4 py-2 sm:py-3 rounded-lg font-medium transition-all whitespace-nowrap flex-shrink-0 text-sm sm:text-base ${
                  activeTab === tab.id
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div className="relative" ref={calendarRef}>
                  ...
                  >
                    ...
                    <span className="text-gray-700 flex-1 text-xs sm:text-sm md:text-base truncate">
                  ...
                  {showCalendar && (
                    <div className="absolute z-50 mt-2 bg-white border border-gray-300 rounded-lg shadow-xl left-1/2 transform -translate-x-1/2 md:left-70 md:transform-none">
                      <DateRange
                        ranges={dateRange}
